...
export default function AlumniContact ({ newEmailArray }) {
  const [isEditing, setIsEditing] = useState(false)
  const [isLoading, setIsLoading] = useState(false)
  const [emailParams, setEmailParams] = useState(
    {
      mail_to: newEmailArray.emails,
      subject: '',
      message: '',
      reply_to: ''
    }
    ...
    <div className='bg-white py-16 px-4 overflow-hidden sm:px-6 lg:px-8 lg:py-24'>
      <div className='relative max-w-xl mx-auto'>
        ...
        <div className='text-center'>
          <h2 className='text-3xl font-extrabold tracking-tight text-gray-900 sm:text-4xl'>Email Alumni</h2>
              ...
              <div className='mt-1'>
                {isEditing
                  ? <textarea
                      ...
                      name='company'
                      id='company'
                      value={newEmailArray.emails.join(', ')}
                  ...
                  : <textarea
                      ...
                      name='company'
                      id='company'
                      value={newEmailArray.names.join(', ')}
                      autoComplete='organization'
                      className='py-3 px-4 block w-full shadow-sm focus:ring-darkerPurple focus:border-darkerPurple border-gray-300 rounded-md'
                      onChange={(e) => handleChange(e.target.name, e.target.value)}
                    />}
