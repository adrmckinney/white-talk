...
import { confirmChangePassword } from '../api/api'
...
export default function PasswordResetConfirm({ token, setToken, setUsername }) {
  ...
  const [validLength, hasNumber, upperCase, lowerCase, match, specialChar, falseCount] =
  ...
  const handlePasswordMatchAlert = () => {
    if (passwordMatchBlur) {
      return <PasswordMatchAlert match={match} />
    }
  }

  const handleConfirmChangePassword = () => {
    setIsLoading(true)
    confirmChangePassword(uid, urlToken, password, confirmPassword).then(data => {
      // logout(token)
      //   .then(data => {
      setIsLoading(false)
      setOpen(false)
